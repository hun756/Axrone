...
const GLTF_SHADER_LIBRARIES = Object.freeze([
    ...
    },
    {
        ...
        code: [
            ...
            '}',
            '',
            'float evaluateSpecularLobe(vec3 normal, vec3 viewDir, vec3 lightDir, float roughness) {',
            ...
            '    float fresnel = mix(0.04, 1.0, pow(1.0 - vdh, 5.0));',
            '    return pow(ndh, specPower) * ndl * fresnel;',
            '}',
            '',
            ...
            '        return vec3(0.0);',
            '    }',
            '    float specular = evaluateSpecularLobe(normal, viewDir, lightDir, roughness);',
            '    vec3 diffuse = albedo * (1.0 - metallic) * ndl;',
            '    vec3 specColor = mix(vec3(0.04), albedo, metallic) * specular;',
            ...
            '    }',
            '    float specular = evaluateSpecularLobe(normal, viewDir, lightDir, roughness);',
            '    return vec3(specular * clearcoat) * lightColor * intensity;',
            '}',
        ],
    },
] as const);
...
const createGltfPbrShaderEffect = (id: string): RenderShaderEffectDefinition => ({
    ...
    fragment: {
        ...
        main: [
            ...
            `        vec3 directionalLight = evaluateLight(normal, viewDir, baseColor.rgb, metallic, roughness, normalize(-${GLTF_LIGHTING_UNIFORMS.directionalLightDirection}[index]), ${GLTF_LIGHTING_UNIFORMS.directionalLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.directionalLightIntensity}[index]);`,
            `        vec3 directionalClearcoat = evaluateClearcoatLight(clearcoatNormal, viewDir, clearcoat, clearcoatRoughness, normalize(-${GLTF_LIGHTING_UNIFORMS.directionalLightDirection}[index]), ${GLTF_LIGHTING_UNIFORMS.directionalLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.directionalLightIntensity}[index]);`,
            '        lighting += directionalLight * baseLayerEnergy + directionalClearcoat;',
            '    }',
            '    lighting += baseColor.rgb * ambient * baseLayerEnergy;',
            ...
            `        vec3 pointLight = evaluateLight(normal, viewDir, baseColor.rgb, metallic, roughness, lightDir, ${GLTF_LIGHTING_UNIFORMS.pointLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.pointLightIntensity}[index] * attenuation);`,
            `        vec3 pointClearcoat = evaluateClearcoatLight(clearcoatNormal, viewDir, clearcoat, clearcoatRoughness, lightDir, ${GLTF_LIGHTING_UNIFORMS.pointLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.pointLightIntensity}[index] * attenuation);`,
            '        lighting += pointLight * baseLayerEnergy + pointClearcoat;',
            '    }',
            ...
            `        vec3 spotLight = evaluateLight(normal, viewDir, baseColor.rgb, metallic, roughness, lightDir, ${GLTF_LIGHTING_UNIFORMS.spotLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.spotLightIntensity}[index] * attenuation);`,
            `        vec3 spotClearcoat = evaluateClearcoatLight(clearcoatNormal, viewDir, clearcoat, clearcoatRoughness, lightDir, ${GLTF_LIGHTING_UNIFORMS.spotLightColor}[index], ${GLTF_LIGHTING_UNIFORMS.spotLightIntensity}[index] * attenuation);`,
            '        lighting += spotLight * baseLayerEnergy + spotClearcoat;',
            '    }',
            '} else {',
            '    lighting = baseColor.rgb * ambient * baseLayerEnergy;',
            '}',
