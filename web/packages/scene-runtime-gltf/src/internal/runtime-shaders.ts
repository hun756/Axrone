...
const GLTF_SHADER_LIBRARIES = Object.freeze([
    ...
    },
    {
        ...
        code: [
            ...
            '    // Attenuate specular environment to prevent desaturated pastel wash',
            '    // caused by raised sky-light defaults. Diffuse IBL keeps full sky contribution.',
            '    const float SPECULAR_ENV_SCALE = 0.6;',
            '    vec3 prefilteredColor = mix(u_GroundLight * 0.5, u_SkyLight, hemiFactor) * SPECULAR_ENV_SCALE;',
            '    float NdotV = max(dot(N, V), 0.001);',
