...
const GLTF_SHADER_LIBRARIES = Object.freeze([
    ...
    },
    {
        ...
        code: [
            ...
            '    float fogFactor = 0.0;',
            '    if (u_FogMode == 0) {',
            '        fogFactor = clamp((u_FogStartEnd.y - dist) / max(u_FogStartEnd.y - u_FogStartEnd.x, 0.001), 0.0, 1.0);',
            '    } else if (u_FogMode == 1) {',
            '        fogFactor = exp(-u_FogDensity * dist);',
            '    } else {',
            '        fogFactor = exp(-u_FogDensity * u_FogDensity * dist * dist);',
            '    }',
    ...
    },
    {
        ...
        code: [
            ...
            '}',
            '',
            'float rangeAttenuation(float distanceToLight, float range, float attenuationFactor) {',
            '    if (range <= 0.0) {',
            '        return 1.0;',
            '    }',
            '    float normalizedDist = distanceToLight / range;',
            '    return 1.0 / (1.0 + attenuationFactor * normalizedDist * normalizedDist);',
            '}',
            '',
            ...
            '}',
            '',
            'vec3 evaluateSpecularBrdf(vec3 N, vec3 V, vec3 L, float roughness, vec3 F0) {',
            ...
            '    vec3 specular = evaluateSpecularBrdf(normal, viewDir, lightDir, roughness, F0);',
    ...
    },
    {
        ...
        code: [
            ...
            '    vec3 irradiance = mix(u_GroundLight, u_SkyLight, hemiFactor) + u_AmbientLight * 0.45;',
            ...
            '    // Physically-correct IBL specular: prefiltered env color modulated by Fresnel and split-sum BRDF.',
            '    vec3 prefilteredColor = mix(u_GroundLight * 0.5, u_SkyLight, hemiFactor);',
            '    float NdotV = max(dot(N, V), 0.001);',
            '    float oneMinusNdotV = 1.0 - NdotV;',
            '    vec2 envBRDF = vec2(',
            '        clamp(pow(roughness, 0.6 + 0.2 * NdotV) * max(oneMinusNdotV, 0.0) * 1.7, 0.0, 1.0),',
            '        clamp(pow(roughness, 2.0 * NdotV) * oneMinusNdotV * 0.5 + 0.02, 0.0, 1.0)',
            '    );',
    ...
    },
    {
        ...
        code: [
            'vec3 evaluateAnisotropicSpecular(vec3 N, vec3 V, vec3 L, float roughness, float anisotropy, vec3 tangent, vec3 bitangent, vec3 F0) {',
...
const createGltfPbrShaderEffect = (id: string): RenderShaderEffectDefinition => ({
    ...
    vertex: {
        ...
        main: [
            ...
            'vec4 worldPosition = u_Model * localPosition;',
            'v_WorldPosition = worldPosition.xyz;',
            'v_WorldNormal = normalize(transpose(inverse(mat3(u_Model))) * localNormal);',
            'v_WorldTangent = vec4(normalize(transpose(inverse(mat3(u_Model))) * localTangent), a_Tangent.w);',
            'gl_Position = u_Projection * u_View * worldPosition;',
        ],
    },
    fragment: {
        precision: 'highp',
        outputs: [{ name: 'o_Color', type: 'vec4' }],
        includes: ['gltf.uv', 'gltf.pbr-lighting', 'gltf.ibl', 'gltf.shadow', 'gltf.advanced-shading', 'gltf.color-space', 'gltf.fog'],
        main: [
            'vec4 baseColor = _BaseColorFactor;',
            'if (_BaseColorTexture_TexCoord >= 0) {',
            '    vec2 uv = transformUV(selectUV(_BaseColorTexture_TexCoord), _BaseColorTexture_ST, _BaseColorTexture_Rotation);',
            ...
            '            spec = evaluateAnisotropicSpecular(normal, viewDir, lightDir, roughness, anisotropy, tangent, bitangent, F0);',
            '        } else {',
            '            spec = evaluateSpecularBrdf(normal, viewDir, lightDir, roughness, F0);',
            '        }',
            '        float NdotL = max(dot(normal, lightDir), 0.0);',
            '        vec3 F = fresnelSchlick(max(dot(normalize(viewDir + lightDir), viewDir), 0.0), F0);',
            ...
            `        float attenuation = rangeAttenuation(distanceToLight, ${GLTF_LIGHTING_UNIFORMS.pointLightRange}[index], ${GLTF_LIGHTING_UNIFORMS.pointLightAttenuation}[index]);`,
            ...
            '        float distanceToLight = length(toLight);',
            '        vec3 lightDir = distanceToLight > 0.0 ? toLight / distanceToLight : vec3(0.0, 1.0, 0.0);',
            `        float attenuation = rangeAttenuation(distanceToLight, ${GLTF_LIGHTING_UNIFORMS.spotLightRange}[index], 2.0);`,
            ...
            'finalColor = applyFog(finalColor, v_WorldPosition, u_CameraPosition);',
            '#endif',
            'o_Color = vec4(linearToSrgb(finalColor), alpha);',
        ],
    },
    renderState: {
        depthTest: true,
...
const createGltfToonShaderEffect = (id: string): RenderShaderEffectDefinition => ({
    ...
    fragment: {
        ...
        main: [
            ...
            `        float atten = rangeAttenuation(dist, ${GLTF_LIGHTING_UNIFORMS.pointLightRange}[index], ${GLTF_LIGHTING_UNIFORMS.pointLightAttenuation}[index]);`,
            `        vec3 lightColor = ${GLTF_LIGHTING_UNIFORMS.pointLightColor}[index];`,
