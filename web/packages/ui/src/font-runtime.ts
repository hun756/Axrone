...
/**
 * Shared base for browser font runtimes that use canvas 2D for measurement
 * and rasterization. Extracts the common glyph cache, kerning cache, metric
 * context, raster canvas/context fields and the measureGlyph/rasterizeGlyph/
 * getKerning/dispose methods (~170 LOC) that were duplicated between
 * BrowserDynamicFontFaceRuntime and BrowserSystemFontFaceRuntime.
 *
 * Subclasses provide only their construction/family-token differences.
 */
abstract class BrowserFontFaceRuntimeBase implements DynamicFontFaceRuntime {
    readonly info: DynamicFontRuntimeInfo;

    protected readonly familyToken: string;
    protected readonly metricContext: CanvasRenderingContext2DLike;
    protected readonly rasterCanvas: CanvasLike;
    protected readonly rasterContext: CanvasRenderingContext2DLike;
    protected readonly glyphs = new Map<number, CachedGlyphMetric>();
    protected readonly kerningCache = new Map<string, number>();
    ...
    /** Hook for subclass-specific disposal (e.g. removing FontFace from document.fonts). */
    ...
    protected ensureActive(): void {
        if (this.disposed) {
            throw new FontLoadError('The font runtime has already been disposed.');
        }
    }
}

class BrowserDynamicFontFaceRuntime extends BrowserFontFaceRuntimeBase {
    ...
    private constructor(
        ...
        super(familyToken, info, metricContext, rasterCanvas, rasterContext);
    ...
    static async create(source: DynamicFontRuntimeSource): Promise<BrowserDynamicFontFaceRuntime> {
        ...
        return new BrowserDynamicFontFaceRuntime(
            quoteFontFamilyToken(familyName),
            fontFace,
            info,
            metricContext,
            rasterPair.canvas,
...
class BrowserSystemFontFaceRuntime extends BrowserFontFaceRuntimeBase {
    private constructor(
        familyToken: string,
        info: DynamicFontRuntimeInfo,
        metricContext: CanvasRenderingContext2DLike,
