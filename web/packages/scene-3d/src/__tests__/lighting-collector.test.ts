...
describe('SceneLightingCollector', () => {
    it('collects stable lighting buffers without per-frame reallocation', () => {
        ...
        expect(second.environment.ambient.x).toBeCloseTo(0.15);
        expect(second.environment.ambient.y).toBeCloseTo(0.24);
        expect(second.environment.ambient.z).toBeCloseTo(0.33);
        ...
        expect([...second.localLightKinds.slice(0, 2)]).toEqual([1, 2]);
