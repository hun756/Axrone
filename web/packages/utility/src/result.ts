...
function asError(e: unknown): Error {
  return e instanceof Error ? e : new ResultError(typeof e === 'string' ? e : formatValue(e), { cause: e });
}

function extractErr<T, E>(r: Err<T, E>): E {
  return r.fold(_ => undefined as never, e => e);
}
...
export class ResultError extends Error {
  ...
  constructor(message: string, options?: { cause?: unknown; code?: string | number }) {
    ...
    const capture = (Error as unknown as { captureStackTrace?: (target: unknown, ctor?: unknown) => void }).captureStackTrace;
...
export interface IResult<T, E> {
  ...
  equals(other: Result<T, E>, valueEq?: (a: T, b: T) => boolean, errEq?: (a: E, b: E) => boolean): boolean;
...
export class Ok<T, E = never> implements IResult<T, E>, Iterable<T> {
  ...
  unwrap(): T { return this._value; }
  getOrElse(_defaultValue: T | Lazy<T>): T { return this._value; }
  getOrDefault(_defaultValue: T): T { return this._value; }
  getOrThrow(): T { return this._value; }
  getOrThrowWith(_transform?: (error: E) => unknown): T { return this._value; }
  unwrapOr(_defaultValue: T): T { return this._value; }
  unwrapOrElse(_fn: (error: E) => T): T { return this._value; }
  expect(_message: string): T { return this._value; }

  expectErr(message: string): E {
    throw new ResultError(message, { cause: this._value });
  }

  toOptional(): T | undefined { return this._value; }
  toNullable(): T | null { return this._value; }
  okToOptional(): T | undefined { return this._value; }
  errToOptional(): E | undefined { return undefined; }
  ...
  containsErr(_error: E): boolean { return false; }

  ok(): Ok<T, E> | null { return this; }
  err(): Err<T, E> | null { return null; }
  ...
  *[Symbol.iterator](): Iterator<T> {
  ...
  equals(other: Result<T, E>, valueEq?: (a: T, b: T) => boolean, _errEq?: (a: E, b: E) => boolean): boolean {
    return other.fold(
...
export class Err<T, E = Error> implements IResult<T, E>, Iterable<never> {
  ...
  getOrDefault(defaultValue: T): T { return defaultValue; }
  ...
  unwrapOrElse(fn: (error: E) => T): T { return fn(this._error); }

  expect(message: string): never {
    throw new ResultError(message, { cause: this._error });
  }

  expectErr(_message: string): E { return this._error; }

  toOptional(): T | undefined { return undefined; }
  toNullable(): T | null { return null; }
  okToOptional(): T | undefined { return undefined; }
  errToOptional(): E | undefined { return this._error; }
  ...
  ok(): Ok<T, E> | null { return null; }
  err(): Err<T, E> | null { return this; }
  ...
  toArray(): T[] { return []; }
  toErrorArray(): E[] { return [this._error]; }
  ...
  equals(other: Result<T, E>, _valueEq?: (a: T, b: T) => boolean, errEq?: (a: E, b: E) => boolean): boolean {
    return other.fold(
      () => false,
...
export function fromJSON<T, E>(json: unknown, reviver?: (value: unknown) => unknown): Result<T, E> {
  ...
  if (typeof json === 'string') {
    try { obj = JSON.parse(json); } catch (e) { return Err.of(asError(e) as unknown as E); }
  }
  ...
  const payload = (obj as { value?: unknown; error?: unknown }).value ?? (obj as { error?: unknown }).error;
...
export class AsyncResult<T, E = Error> implements PromiseLike<Result<T, E>> {
  ...
  map<U>(fn: (value: T) => U | Promise<U>): AsyncResult<U, E> {
    return AsyncResult.from(this._promise.then(async r =>
  ...
  flatMap<U, F = E>(fn: (value: T) => Result<U, F> | AsyncResult<U, F> | Promise<Result<U, F>>): AsyncResult<U, F> {
    return AsyncResult.from(this._promise.then(async r => {
  ...
  andThen<U, F = E>(fn: (value: T) => Result<U, F> | AsyncResult<U, F> | Promise<Result<U, F>>): AsyncResult<U, F> {
  ...
  recover<F>(fn: (error: E) => Result<T, F> | AsyncResult<T, F> | Promise<Result<T, F>>): AsyncResult<T, F> {
    return this.orElse(fn);
  }

  fold<U>(onOk: (value: T) => U | Promise<U>, onErr: (error: E) => U | Promise<U>): Promise<U> {
    return this._promise.then(async r =>
      r.isOk() ? onOk(r.get()) : onErr(extractErr(r))
    );
  }
