...
describe('PerfettoSerializer', () => {
  ...
  describe('recordSample()', () => {
    ...
    it('should create B/E event pairs for each frame', () => {
      serializer.recordSample([makeFrame({ function: 'funcA' }), makeFrame({ function: 'funcB' })]);
      const data = serializer.getPerfettoData()!;
      const beEvents = data.events.filter((e: { ph: string }) => e.ph === 'B' || e.ph === 'E');
    ...
    it('should set ph to x when durationNs is provided', () => {
      ...
      const data = serializer.getPerfettoData()!;
      const sampleEvent = data.events.find((e: { name: string }) => e.name.startsWith('sample.'));
    ...
    it('should include lineNumber in B event args', () => {
      ...
      const data = serializer.getPerfettoData()!;
      const bEvent = data.events.find((e: { ph: string; name: string }) => e.ph === 'B' && e.name === 'funcA');
