...
export class LogBoundedHistogram {
    ...
    private findBucket(value: number): number {
        for (let i = 0; i < this.bucketBoundaries.length - 1; i++) {
            if (
                value >= this.bucketBoundaries[i] &&
                (i + 1 >= this.bucketBoundaries.length || value < this.bucketBoundaries[i + 1])
            ) {
                return i;
            }
        }
    ...
    getStatistics(): Record<string, unknown> {
        ...
        for (let i = 0; i < this.countsPerBucket.length; i++) {
            if (this.countsPerBucket[i] > 0n) {
                sorted.push({
                    value: this.bucketBoundaries[i] ?? 0,
                    cumulativeCount: cumulative + this.countsPerBucket[i],
                });
    ...
    private computePercentile(
        sorted: Array<{ value: number; cumulativeCount: bigint }>,
        total: bigint,
        p: number
    ): number | undefined {
