...
export function decomposeSwingTwist(
    ...
    // Canonicalize the quaternion sign (q and -q are the same rotation) so the
    // extracted angles are continuous across the w = 0 boundary.
    ...
    // Twist projection onto the frame X axis.
    const p = qx;
    ...
    if (norm2 > EPSILON * EPSILON) {
        ...
        // qSwing = qRel * conj(qTwist), with conj(qTwist) = (-twX, 0, 0, twW).
        // Quaternion product (w1, v1) * (w2, v2):
        //   w = w1*w2 - v1·v2,  v = w1*v2 + w2*v1 + v1×v2
        swingW = qw * twW - (qx * -twX);
        swingX = qw * -twX + twW * qx; // + (qy*0 - qz*0) = 0 term
        swingY = twW * qy + (qz * -twX - qx * 0);
        swingZ = twW * qz + (qx * 0 - qy * -twX);

        out.twistAngle = 2.0 * Math.atan2(twX, twW);
    } else {
        // 180° swing about an axis perpendicular to the twist axis: the twist
        // component is undefined — treat as zero twist, full swing.
        out.twistAngle = 0;
    }
