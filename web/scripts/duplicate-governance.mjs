...
const glslMarkerPattern = /(?:#version\s|precision\s+(?:low|med|high)p|\bvec[234]\b|\bfloat\s+\w+\s*\(|\bmat[234]\b|\bu?ivec[234]\b|\bsampler(?:2D|3D|Cube)\b|\bvoid\s+main\s*\(\s*\)\s*\{|\bin\s+(?:vec[234]|mat[234]|float|int|u?ivec[234])\s+\w+)/;
